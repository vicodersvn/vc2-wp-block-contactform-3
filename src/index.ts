...
export default function handler(options: any): Rule {
  const templateSource = apply(url('./files/contactform'), [
    applyTemplates({
      ...strings,
      ...options
    }),
  ...
  return chain([
    ...
    App.make(WordpressService).declareInServiceProvider('app/Providers/BlockServiceProvider.php', `\\App\\Blocks\\Contact2Block::class,`)
  ]);
}
